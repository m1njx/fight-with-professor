...
const BOSS_HP_LIST   = [500, 1000, 1500, 2000];
const PLAYER_ATK     = [10, 20, 30, 40];
const BOSS_ATK       = 10;

// 라운드별 AI 설정
const AI_CONFIG = [
    { attackInterval: 180, blockChance: 0.1, aggressiveness: 0.3 },
    { attackInterval: 130, blockChance: 0.2, aggressiveness: 0.5 },
    { attackInterval:  90, blockChance: 0.3, aggressiveness: 0.7 },
    { attackInterval:  60, blockChance: 0.4, aggressiveness: 0.9 }
];

// 배경 색상 (4라운드)
const BG_THEMES = [
    { sky: '#1a1a2e', ground: '#16213e', accent: '#0f3460', name: '강의실' },
    { sky: '#2d1b00', ground: '#1a0f00', accent: '#8b4513', name: '도서관' },
    { sky: '#0d2137', ground: '#071520', accent: '#1a4a6b', name: '운동장' },
    { sky: '#1a0030', ground: '#0d0018', accent: '#4a0080', name: '졸업식장' }
];
...
function drawBgDecoration(theme) {
    ...
    if (currentRound === 0) {
        ...
        ctx.lineWidth   = 3;
        ctx.fillRect(200, 60, 500, 160);
        ctx.strokeRect(200, 60, 500, 160);

        ctx.fillStyle = '#ffffff33';
        ctx.font      = '18px Courier New';
        ctx.fillText('시험 범위: 전체', 280, 110);
        ctx.fillText('출석 점수: 0점', 280, 145);
        ctx.fillText('F학점 경고 ⚠️', 280, 180);
        ...
        for (let i = 0; i < 6; i++) {
            ...
            ctx.strokeRect(bx, 50, 100, 200);

            // 책들
            const bookColors = [
                '#cc2200','#0044cc','#00aa44',
                '#cc8800','#8800cc','#cc0088'
            ];
            for (let j = 0; j < 5; j++) {
                ctx.fillStyle = bookColors[(i + j) % bookColors.length];
                ctx.fillRect(bx + 8 + j * 17, 60, 14, 180);
            }
        }
        ...
        for (let i = 0; i < 4; i++) {
            ...
            ctx.arc(cx + 40, 90,  50, 0, Math.PI * 2);
            ctx.arc(cx + 80, 100, 35, 0, Math.PI * 2);
            ctx.fill();
        }
        ...
        ctx.lineWidth   = 3;
        ctx.fillRect(150, 80, 600, 180);
        ctx.strokeRect(150, 80, 600, 180);

        // 커튼
        ctx.fillStyle = '#440088';
        ctx.fillRect(150, 80, 60, 180);
        ctx.fillRect(690, 80, 60, 180);

        // 조명
        const spotColors = ['#ffdd0044','#ff440044','#00ccff44'];
        for (let i = 0; i < 3; i++) {
            const lx = 280 + i * 160;
        ...
        ctx.fillText('🎓 졸업 축하합니다 🎓', canvas.width / 2, 140);
        ctx.textAlign = 'left';
    }
