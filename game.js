...
document.getElementById('gameOverRestartBtn').onclick = () => {
    // 페이지 새로고침 (입력 화면으로 돌아가기)
    location.reload();
};
document.getElementById('menuBtn').onclick = handleMenuReturn;
...
// 라운드별 AI 설정 (라운드 진행에 따라 난이도 상승)
const AI_CONFIG = [
    { attackInterval: 100, blockChance: 0.07, aggressiveness: 0.4 },  // Round 1: 공격 빈도 증가
    { attackInterval:  75, blockChance: 0.14, aggressiveness: 0.6 },  // Round 2: 더 자주 공격
    { attackInterval:  55, blockChance: 0.21, aggressiveness: 0.8 },  // Round 3: 매우 공격적
    { attackInterval:  40, blockChance: 0.28, aggressiveness: 0.95 }  // Round 4: 극도로 공격적
];
...
function handleStart() {
    ...
    // ✅ 필수 입력값 검증
    if (!pName || !bName || !sName) {
        let missingFields = [];
        if (!pName) missingFields.push('플레이어 이름');
        if (!bName) missingFields.push('교수님 성함');
        if (!sName) missingFields.push('학교 이름');
    ...
    // 🎨 고급 그래픽 활성화 (graphics-upgrade.js 로드 후)
    if (typeof drawBackground_Premium !== 'undefined') {
        window.drawBackground = drawBackground_Premium;
    }
    if (typeof spawnHitEffect_Premium !== 'undefined') {
        window.spawnHitEffect = spawnHitEffect_Premium;
    }
    if (typeof drawHitEffects_Premium !== 'undefined') {
        window.drawHitEffects = drawHitEffects_Premium;
    }

    // 학생 상태별 프리미엄 드로잉이 있는 경우만 덮어쓰기
    if (typeof drawStudentIdle_Premium !== 'undefined') {
        window.drawStudentIdle = drawStudentIdle_Premium;
    }
    if (typeof drawStudentWalk_Premium !== 'undefined') {
        window.drawStudentWalk = drawStudentWalk_Premium;
    }
    if (typeof drawStudentJump_Premium !== 'undefined') {
        window.drawStudentJump = drawStudentJump_Premium;
    }
    if (typeof drawStudentAttack_Premium !== 'undefined') {
        window.drawStudentAttack = drawStudentAttack_Premium;
    }
    if (typeof drawStudentHurt_Premium !== 'undefined') {
        window.drawStudentHurt = drawStudentHurt_Premium;
    }
    if (typeof drawStudentGuard_Premium !== 'undefined') {
        window.drawStudentGuard = drawStudentGuard_Premium;
    }
    if (typeof drawStudentDown_Premium !== 'undefined') {
        window.drawStudentDown = drawStudentDown_Premium;
    }

    // 교수 상태별 프리미엄 드로잉이 있는 경우만 덮어쓰기
    if (typeof drawProfIdle_Premium !== 'undefined') {
        window.drawProfIdle = drawProfIdle_Premium;
    }
    if (typeof drawProfWalk_Premium !== 'undefined') {
        window.drawProfWalk = drawProfWalk_Premium;
    }
    if (typeof drawProfJump_Premium !== 'undefined') {
        window.drawProfJump = drawProfJump_Premium;
    }
    if (typeof drawProfAttack_Premium !== 'undefined') {
        window.drawProfAttack = drawProfAttack_Premium;
    }
    if (typeof drawProfHurt_Premium !== 'undefined') {
        window.drawProfHurt = drawProfHurt_Premium;
    }
    if (typeof drawProfGuard_Premium !== 'undefined') {
        window.drawProfGuard = drawProfGuard_Premium;
    }
    if (typeof drawProfDown_Premium !== 'undefined') {
        window.drawProfDown = drawProfDown_Premium;
    }

    if (typeof drawBackground_Premium !== 'undefined' ||
        typeof drawStudentIdle_Premium !== 'undefined' ||
        typeof drawProfIdle_Premium !== 'undefined') {
        console.log('✨ 고급 그래픽이 활성화되었습니다!');
    }
...
// ================================================
//  메뉴로 돌아가기 (중간에 게임 빠져나오기)
// ================================================
async function handleMenuReturn() {
    // 게임 상태 중단
    gameRunning = false;
    roundPause = true;
    ...
    // 현재까지의 진행도 저장 (도달한 라운드까지의 기록)
    try {
        const schoolName = document.getElementById('schoolName').value.trim();
        const reachedRound = currentRound + 1; // 현재 라운드 + 1 (도달한 라운드)

        // 라운드 결과가 없으면 진행중으로 기록
        const finalResults = [...roundResults];
        if (finalResults.length <= currentRound) {
            finalResults.push('PROGRESS');
        }
        ...
        const gameData = {
            ...
            clearRound: reachedRound,
            results: finalResults,
            clearedAt: new Date()
        };

        console.log('📊 게임 진행도 저장:', gameData);
        await saveRecord(gameData);

        // 성공 알림
        console.log('✅ 게임 데이터가 저장되었습니다!');
    } catch (error) {
        console.error('⚠️ 게임 데이터 저장 중 오류:', error);
        // 오류가 발생해도 계속 진행 (화면 전환은 하기)
    }
    ...
    try {
        ...
        console.log('✅ 랭킹 화면이 새로고침되었습니다!');
    } catch (error) {
        console.error('⚠️ 랭킹 새로고침 중 오류:', error);
    }
}
...
function dealDamage(target, amount, isPlayer) {
    ...
    // 화면 진동 추가
    screenShake = Math.min(amount / 5, 15);
...
function updateAI() {
    ...
    if (boss.state !== 'attack' && boss.state !== 'special' &&
        ...
        const moveRng = Math.random();
        const aggr    = cfg.aggressiveness;
        
        // 라운드별 이동 속도 증가
        const baseSpeed = 2 + currentRound * 0.7;
        ...
        if (dist > 180) {
            ...
            // 도망 (뒤로물러남)
            boss.vx    = -boss.facing * (baseSpeed + 0.5);
            boss.state = 'walk';
        } else {
            // 중간거리: 무빙 & 회피 전략
            if (moveRng < aggr * 0.4) {
                // 옆으로 움직이며 회피
                boss.vx    = (moveRng < aggr * 0.2 ? 1 : -1) * baseSpeed * 0.7;
                boss.state = 'walk';
            } else if (moveRng < aggr * 0.6) {
                // 상대로 접근
                boss.vx    = boss.facing * baseSpeed * 0.8;
                boss.state = 'walk';
            } else if (moveRng < aggr * 0.85) {
                // 거리 유지
                boss.vx    = (Math.random() - 0.5) * baseSpeed * 0.5;
                boss.state = moveRng < aggr * 0.82 ? 'walk' : 'idle';
            } else {
                // 직진
                boss.vx    = boss.facing * baseSpeed;
                boss.state = 'walk';
    ...
    // 점프 (랜덤 - 이전보다 더 자주)
    if (boss.onGround && Math.random() < 0.008 * (currentRound + 1)) {
        boss.vy       = -12 - currentRound * 0.5;  // 라운드 진행에 따라 더 높이 점프
        boss.onGround = false;
        boss.state    = 'jump';
    }

    // 추가: 회피 점프 (플레이어 공격 감지 시)
    if (boss.onGround && player.state === 'attack' && dist < 120 && 
        Math.random() < cfg.aggressiveness * 0.15) {
        boss.vy       = -11;
        boss.onGround = false;
        boss.state    = 'jump';
    }
...
//  클리어 & 게임오버
// ================================================
// showClear() 및 showGameOver()는 파일 하단에 정의된 최신 버전을 사용합니다.
...
async function showClear() {
    ...
    clearStats.innerHTML = `
        ...
        <div class="stat-row">
            <span>최종 결과</span>
            <span style="color:#ffdd00; font-weight:bold;">🎓 시험지 획득 성공!</span>
        </div>
        <div class="stat-row">
            <span>상대 교수님</span>
            <span>${bossName}</span>
    ...
    // ✅ 클리어 화면에 랭킹 버튼 추가 (만약 아직 없다면)
    if (!document.getElementById('rankingBtnInClear')) {
        document.getElementById('clearRestartBtn').insertAdjacentHTML(
            'afterend',
            `<button
                id="rankingBtnInClear"
                onclick="openRankingScreen('${playerSchool || ''}')"
                style="
                    margin-top:10px;
                    padding:12px 40px;
                    font-size:16px;
...
async function showGameOver() {
    ...
    try {
        ...
        const roundNum = Math.max(1, currentRound); // 최소 1라운드
        
        console.log(`📊 게임 오버 기록 저장: ${roundNum} 라운드에서 패배`);
        ...
        console.warn('⚠️ 패배 기록 저장 실패 (계속 진행...):', error);
    }
}
