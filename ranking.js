...
function getDb() {
    if (!window.db) {
        console.warn('⚠️ Firebase DB가 아직 초기화되지 않았습니다. window.db를 확인하세요.');
    }
    return window.db;
}
...
async function saveRecord(data) {
    console.log('💾 기록 저장 시작...', data);
    const db = getDb();
    if (!db) {
        console.error('❌ Firebase DB 초기화 실패: 저장할 수 없습니다.');
        return;
    }
    try {
        await addDoc(collection(db, 'records'), {
            playerName : data.playerName || '익명',
            school     : data.school     || '미소속',
            totalDmg   : Number(data.totalDmg) || 0,
            clearRound : data.clearRound || 0,
...
function renderRankingTable(records, containerId) {
    ...
    const rows = records.map(r => {
        ...
        const dmg = (r.totalDmg || 0).toLocaleString();
        const roundVal = r.clearRound || 0;
        const roundText = roundVal >= 4 ? '<span style="color:#ffdd00; font-weight:bold;">시험지 획득</span>' : `${roundVal}R`;

        return `
            <tr class="rank-row ${r.rank <= 3 ? 'top-rank' : ''}">
                <td class="rank-medal">${medal}</td>
                <td class="rank-name">${r.playerName || '익명'}</td>
                <td class="rank-school">${r.school || '-'}</td>
                <td class="rank-dmg">${dmg}</td>
                <td class="rank-round">${roundText}</td>
...
async function initRankingPreview() {
    ...
    console.log('📌 DB 상태:', getDb() ? '✅ 초기화됨' : '❌ 미초기화');
...
window.initRankingPreview = initRankingPreview;
