...
async function loadRanking(schoolName = '') {
    ...
    try {
        ...
        container.innerHTML = '<div class="rank-empty">랭킹 로드 실패 😞<br>잠시 후 다시 시도하세요</div>';
    }
}
...
async function initRankingPreview() {
    ...
    if (!db) {
        container.innerHTML = '<div class="rank-empty">랭킹 시스템 준비 중... 🔄</div>';
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(async () => {
        if (!window.db) {
            ...
            const container = document.getElementById('rankingTableWrap');
            if (container) {
                container.innerHTML = '<div class="rank-empty">랭킹 시스템 연결 실패 🔌<br>페이지를 새로고침 해주세요</div>';
            }
            return;
        }
        try {
